...
class Sddr(object):
    ...
    def train(self, target, structured_data, unstructured_data=dict(), resume=False, plot=False):
        ...
        self.train_loader = DataLoader(train, 
                                    ...
                                    sampler = torch.data.samplers.SequentialSamplerSequentialSampler(train, batch_size=self.config['train_parameters']['batch_size'], drop_last=False),
                                    num_workers=self.config['train_parameters']['num_workers'])
        self.val_loader = DataLoader(val, 
                                    batch_size=self.config['train_parameters']['batch_size'],
                                    collate_fn=collate_pad,
                                    sampler = torch.data.samplers.SequentialSamplerSequentialSampler(val, batch_size=self.config['train_parameters']['batch_size'], drop_last=False),
                                    num_workers=self.config['train_parameters']['num_workers'])
